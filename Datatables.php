  ...
  class Datatables
  {
    ...
    */
    protected function produce_output($charset) {
      ...
      $i=0;
      $dat=$rResult->result_array();
     // print_r($dat);
      foreach($dat as $results){      
	$url_param = $results['employee_code'];
	if($results['status'] == 1){
	  $dat[$i]['status']='<span style="color:green">Active</span>';
	}
	else
	{
	  $dat[$i]['status']='<span style="color:red">Deactive</span>';
	}	
	$dat[$i]['view']='<a class="mm_view" id="view" onclick="view_empl(this)" name="'.base_url().'index.php/admin/emp_view/'.$url_param.'" href="#">View</i></a>'; 
	$dat[$i]['action']='<a href="#" onclick="add_emp1(this)" class="add_emp_edit ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only" id="create-user" name="'.base_url().'index.php/admin/add_emp/'.$url_param.'"><span class="ui-button-text"><i class="icon-edit"></i></span></a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="'.base_url().'index.php/admin/delete/'.$url_param.'" role="button" data-toggle="modal"><i class="icon-remove"></i></a>'; 
	$i++;
      }
      //print_r($dat); .
  
      
      foreach($dat as $row_key => $row_val)
      {
